...
export const Navbar: React.FC<NavbarProps> = () => {
  ...
  // Auto-close on scroll & escape key
  useEffect(() => {
    const handleScroll = () => {
      setScrolled(window.scrollY > 20);
      
      // Auto close mobile menu when user scrolls
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          <button
            ...
            className="lg:hidden p-2.5 rounded-full bg-white border border-slate-200 text-slate-700 hover:text-slate-900 shadow-sm transition-colors"
            aria-label="Toggle menu"
          >
            {mobileMenuOpen ? <X className="w-5 h-5 text-slate-950" /> : <Menu className="w-5 h-5" />}
          </button>
        </div>
      </header>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40">
          {/* Opaque dark blur overlay that covers entire screen to prevent bleed-through */}
          <div 
            className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm transition-opacity duration-300"
            onClick={() => setMobileMenuOpen(false)}
          />

          {/* 100% Solid White Drawer Container */}
          <div className="fixed inset-x-0 top-[60px] bg-white border-b border-slate-200 p-6 shadow-2xl z-50 max-h-[calc(100vh-70px)] overflow-y-auto space-y-4">
            ...
            <div className="pt-4 border-t border-slate-100 flex flex-col gap-2.5">
              <a
                ...
                rel="noopener noreferrer"
                onClick={() => setMobileMenuOpen(false)}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-full bg-slate-100 hover:bg-blue-50 hover:text-blue-600 text-slate-800 text-sm font-bold transition-colors shadow-sm"
              >
              ...
              <a
                href="#contact"
                onClick={(e) => handleNavClick(e, '#contact')}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-full bg-slate-950 text-white text-sm font-bold shadow-md hover:bg-blue-600 transition-colors"
              >
